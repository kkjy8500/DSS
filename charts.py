...
def render_population_box(pop_df: pd.DataFrame):
    ...
    with box:
        ...
        mid_col    = next((c for c in ["40_59비율","40-59비율","age40_59p","4050"] if c in pop_df.columns), None)
        male_col   = next((c for c in ["남성비율","남","male_p","2030 남성"] if c in pop_df.columns), None)
        female_col = next((c for c in ["여성비율","여","female_p","2030 여성"] if c in pop_df.columns), None)
        total_voter= next((c for c in ["유권자수","유권자 수","voters","전체 유권자"] if c in pop_df.columns), None)
